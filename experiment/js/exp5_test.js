...
function directDelete(element) {
	...
	if (deletemode == 1) {
		...
		if (toDelete.id.includes('persist')) {
			// some element does not allow to delete
			// alert('請勿刪除原有的元件');
			return;
		}
		if (toDelete.id[0] == 'w') {
...
function equationVoltageVoltage() {
	const FG = getFullGraphVoltageVoltage();
	graph = FG.graph;
	equations = [];
	equation_cnt = 0;
	vis_edge = [];
...
function checkCircuit() {
	const FGx = equationVoltageVoltage();
...
function start() {
	...
	document.getElementById('svgline').appendChild(parseSVG(`<line dataohm="${ohms}"id=resistance0${resistanceNo}_persist x1=${x1} y1=${y1} x2=${x2} y2=${y2} style="stroke:${colorlist[colorNo]};stroke-width:2" onclick="directDelete(this)"><title>電阻 B</title></line>`));
	document.getElementById('svgline').appendChild(parseSVG(`<polygon id=resistanceBox0${resistanceNo}_persist points="${rectX1},${rectY1} ${rectX2},${rectY2} ${rectX3},${rectY3} ${rectX4},${rectY4}" style="fill:blue; stroke:lime; stroke-width:1" onclick="directDelete(this)"><title>電阻 B</title></polygon>`));
	findPersistNode();
}
function checkAns() {
	if (!startbool)
