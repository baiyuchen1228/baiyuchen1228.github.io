...
const voltage2 = 0;
const power = 0;
const powersupplyOutputStatus = 0;
...
$('#container').mouseup((e) => {
	...
	if (drawCapacitance) {
		...
		var rectY4 = centerY - 10 * Math.sin(slope) + 5 * Math.cos(slope);
		if (capacitanceNo < 10) {
			const circle1 = parseSVG(`<circle id=capacitanceCircle1_0${capacitanceNo} cx=${x1} cy=${y1} r=${5} style="fill:${colorlist[colorNo]};stroke-width:2"><title></title></line>`, directDelete);
			const circle2 = parseSVG(`<circle id=capacitanceCircle2_0${capacitanceNo} cx=${x2} cy=${y2} r=${5} style="fill:${colorlist[colorNo]};stroke-width:2"><title></title></line>`, directDelete);
			const line = parseSVG(`<line dataufarad="${ufarad}"id=capacitance0${capacitanceNo} x1=${x1} y1=${y1} x2=${x2} y2=${y2} style="stroke:${colorlist[colorNo]};stroke-width:2"><title>${ufarad * 1e6}uFarad</title></line>`, directDelete);
			const box = parseSVG(`<polygon id=capacitanceBox0${capacitanceNo} points="${rectX1},${rectY1} ${rectX2},${rectY2} ${rectX3},${rectY3} ${rectX4},${rectY4}" style="fill:rgb(255,0,0); stroke:black; ;stroke-width:1" ><title>${ufarad * 1e6}uFarad</title></polygon>`, directDelete);
			document.getElementById('svgline').appendChild(circle1);
			document.getElementById('svgline').appendChild(circle2);
			document.getElementById('svgline').appendChild(line);
			document.getElementById('svgline').appendChild(box);
			...
			const circle1 = parseSVG(`<circle id=capacitanceCircle1_${capacitanceNo} cx=${x1} cy=${y1} r=${5} style="fill:${colorlist[colorNo]};stroke-width:2"><title></title></line>`, directDelete);
			const circle2 = parseSVG(`<circle id=capacitanceCircle2_${capacitanceNo} cx=${x2} cy=${y2} r=${5} style="fill:${colorlist[colorNo]};stroke-width:2"><title></title></line>`, directDelete);
			const line = parseSVG(`<line dataufarad="${ufarad}"id=capacitance${capacitanceNo} x1=${x1} y1=${y1} x2=${x2} y2=${y2} style="stroke:${colorlist[colorNo]};stroke-width:2"><title>${ufarad * 1e6}uFarad</title></line>`, directDelete);
			const box = parseSVG(`<polygon id=capacitanceBox${capacitanceNo} points="${rectX1},${rectY1} ${rectX2},${rectY2} ${rectX3},${rectY3} ${rectX4},${rectY4}" style="fill:rgb(255,0,0); stroke:black; ;stroke-width:1" ><title>${ufarad * 1e6}uFarad</title></polygon>`, directDelete);
			document.getElementById('svgline').appendChild(circle1);
			document.getElementById('svgline').appendChild(circle2);
			document.getElementById('svgline').appendChild(line);
			document.getElementById('svgline').appendChild(box);
...
function getAlligator() {
	...
	const offsetX = 100; // 550;
	const offsetY = 420; // 300;
...
function getResistance() {
	...
	for (let i = 0; i < resistanceOut.length; i++) {
		...
		r.val = math.complex(Number.parseInt(r.val), 0);
...
function getCapacitances(omega) {
	// find all resistance in the html
	const capacitances = $('line[id^=\'capacitance\']');
	const capacitanceOut = $.map(capacitances, (capacitance) => {
		const rval = $(`#${capacitance.id}`).attr('dataufarad');
		return {
			id: capacitance.id,
			val: rval,
	...
	for (let i = 0; i < capacitanceOut.length; i++) {
		const r = capacitanceOut[i];
		r.val = Number.parseFloat(r.val);
...
function getInductances(omega) {
	// find all resistance in the html
	const inductances = $('line[id^=\'inductance\']');
	const inductanceOut = $.map(inductances, (inductance) => {
		const rval = $(`#${inductance.id}`).attr('datahenry');
		return {
			id: inductance.id,
			val: rval,
...
function getFullGraph(graph, meter_idx, omega, checkUser) {
	...
	for (let i = 0; i < wires.length; i++) {
		const wire = wires[i];
		const e = new Edge(wire.node1, wire.node2, 'wire', math.complex(1, 0));
		edge_list.push(e);
	...
	for (let i = 0; i < alligators.length; i++) {
		...
		const e = new Edge(alli.node1, alli.node2, 'wire', math.complex(1, 0));
		edge_list.push(e);
	...
	const capacitances = getCapacitances(omega);
	for (let i = 0; i < capacitances.length; i++) {
		const r = capacitances[i];
		const e = new Edge(r.node1, r.node2, 'capacitance', r.val);
		edge_list.push(e);
		graph[r.node1].push(e);
		graph[r.node2].push(e);
	}

	const inductances = getInductances(omega);
	for (let i = 0; i < inductances.length; i++) {
		const r = inductances[i];
		const e = new Edge(r.node1, r.node2, 'inductance', r.val);
		edge_list.push(e);
		graph[r.node1].push(e);
		graph[r.node2].push(e);
	}
	...
	if (meter_idx == 0) {
		if (checkUser == false) {
			// 接地要 short
			const tmp = new Edge(1, 3, 'wire', math.complex(0, 0));
			edge_list.push(tmp);
			graph[1].push(tmp);
			graph[3].push(tmp);
		}

		const e = new Edge(3, 2, 'voltmeter', math.complex(100000000, 0));
		edge_list.push(e);
		graph[2].push(e);
		graph[3].push(e);
		return { graph, voltage_edgeid: e.id };
	} else {
		if (checkUser == false) {
			// 接地要 short
		...
		const e = new Edge(5, 4, 'voltmeter', math.complex(100000000, 0));
		edge_list.push(e);
		graph[4].push(e);
		graph[5].push(e);
		return { graph, voltage_edgeid: e.id };
...
function getFullGraphVoltageVoltage(meter_idx, omega, checkUser) {
	...
	// if(checkUser == false){
	// 加 wave generator
	const e = new Edge(0, 1, 'voltage source', math.complex(1, 0));
	edge_list.push(e);
	graph[0].push(e);
	graph[1].push(e);
	// }
...
function equationVoltageVoltage(meter_idx, omega) {
	const FG = getFullGraphVoltageVoltage(meter_idx, omega, false);
	graph = FG.graph;
...
function checkConnected() {
	const res = { voltage1: 1, voltage2: 1 };
...
function checkCircuit(omega) {
	const res_meter = { voltage1: 0, voltage2: 0 };

	const FGx = equationVoltageVoltage(0, omega);
	...
	const FGx2 = equationVoltageVoltage(1, omega);
	const FG2 = FGx2.FullGraph;
	const x2 = FGx2.ans;
	if (checkMeter(x2)) {
...
let id;
const faradlist = [1e-6, 1.1e-6, 1.2e-6, 1.3e-6];
const henrylist = [0.02, 0.01, 0.02, 0.01];
function start() {
	console.log('Starting');

	// check id input
	id = Number.parseInt($('#id1')[0].value, 10);
	if (isNaN(id)) {
	...
	let x2 = 205;
	let y2 = 225;
	const capacitanceNo = 3;
	const ufarad = faradlist[id % 4];
	var centerX = x1 - (x1 - x2) / 2;
	var centerY = y1 - (y1 - y2) / 2;
	var slope = Math.atan((y2 - y1) / (x2 - x1));
	var rectX1 = centerX - 5 * Math.sin(slope) + 10 * Math.cos(slope);
	...
	x2 = 225;
	y2 = 125;
	const henry = henrylist[3];
	const inductanceNo = 4;
	var centerX = x1 - (x1 - x2) / 2;
	var centerY = y1 - (y1 - y2) / 2;
	var slope = Math.atan((y2 - y1) / (x2 - x1));
	var rectX1 = centerX - 5 * Math.sin(slope) + 10 * Math.cos(slope);
	...
	var rectX4 = centerX - 10 * Math.cos(slope) - 5 * Math.sin(slope);
	var rectY4 = centerY - 10 * Math.sin(slope) + 5 * Math.cos(slope);
	document.getElementById('svgline').appendChild(parseSVG(`<circle id=inductanceCircle1_0${inductanceNo}_persist cx=${x1} cy=${y1} r=${5} style="fill:${colorlist[colorNo]};stroke-width:2" onclick="directDelete(this)"><title></title></line>`));
	document.getElementById('svgline').appendChild(parseSVG(`<circle id=inductanceCircle2_0${inductanceNo}_persist cx=${x2} cy=${y2} r=${5} style="fill:${colorlist[colorNo]};stroke-width:2" onclick="directDelete(this)"><title></title></line>`));
	document.getElementById('svgline').appendChild(parseSVG(`<line datahenry="${henry}"id=inductance0${inductanceNo}_persist x1=${x1} y1=${y1} x2=${x2} y2=${y2} style="stroke:${colorlist[colorNo]};stroke-width:2" onclick="directDelete(this)"><title>${henry}Henry</title></line>`));
	document.getElementById('svgline').appendChild(parseSVG(`<polygon id=inductanceBox0${inductanceNo}_persist points="${rectX1},${rectY1} ${rectX2},${rectY2} ${rectX3},${rectY3} ${rectX4},${rectY4}" style="fill:rgb(255,215,0); stroke:black; stroke-width:1" onclick="directDelete(this)"><title>${henry}Henry</title></polygon>`));
	...
	var rectY4 = centerY - 10 * Math.sin(slope) + 5 * Math.cos(slope);
	const ohms = 20;
	const resistanceNo = 6;
	document.getElementById('svgline').appendChild(parseSVG(`<circle id=resistanceCircle1_0${resistanceNo}_persist cx=${x1} cy=${y1} r=${5} style="fill:${colorlist[colorNo]};stroke-width:2" onclick="directDelete(this)"><title></title></line>`));
	document.getElementById('svgline').appendChild(parseSVG(`<circle id=resistanceCircle2_0${resistanceNo}_persist cx=${x2} cy=${y2} r=${5} style="fill:${colorlist[colorNo]};stroke-width:2" onclick="directDelete(this)"><title></title></line>`));
	document.getElementById('svgline').appendChild(parseSVG(`<line dataohm="${ohms}"id=resistance0${resistanceNo}_persist x1=${x1} y1=${y1} x2=${x2} y2=${y2} style="stroke:${colorlist[colorNo]};stroke-width:2" onclick="directDelete(this)"><title>${ohms}Ohms</title></line>`));
	document.getElementById('svgline').appendChild(parseSVG(`<polygon id=resistanceBox0${resistanceNo}_persist points="${rectX1},${rectY1} ${rectX2},${rectY2} ${rectX3},${rectY3} ${rectX4},${rectY4}" style="fill:blue; stroke:lime; stroke-width:1" onclick="directDelete(this)"><title>${ohms}Ohms</title></polygon>`));
...
function checkAns() {
	...
	const answer1 = 1 / 2 / Math.PI / Math.sqrt(henrylist[3] * faradlist[id % 4]);
	if (isNaN(ans1)) {
		done = false;
	}
	if (abs(ans1 - answer1) > answer1 * 0.03) {
		done = false;
	}
	if (done) {
...
function minus_vertical_v_outer1() {
	...
	const text = `${v_outer_dis[v_outer_ind_1]}V`;
	$('#vertical_v1').text(text);
	// check();
	osi.draw();
}
function add_vertical_v_outer1() {
	if (v_outer_ind_1 == v_outer_dis.length - 1)
		return;
	v_outer_ind_1++;
	osi.set_vertical_v(0, v_outer_dis[v_outer_ind_1]);
	const text = `${v_outer_dis[v_outer_ind_1]}V`;
...
function minus_vertical_v_outer2() {
	...
	const text = `${v_outer_dis[v_outer_ind_2]}V`;
	$('#vertical_v2').text(text);
	// check();
	osi.draw();
}
function add_vertical_v_outer2() {
	if (v_outer_ind_2 == v_outer_dis.length - 1)
		return;
	v_outer_ind_2++;
	osi.set_vertical_v(1, v_outer_dis[v_outer_ind_2]);
	const text = `${v_outer_dis[v_outer_ind_2]}V`;
...
function minus_horizonal_time() {
	...
	const text = `${time_mul_des[time_mul_ind]}ms`;
	$('#horizonal_time').text(text);
	// check();
	osi.draw();
}
function add_horizonal_time() {
	if (time_mul_ind == time_mul_des.length - 1)
		return;
	time_mul_ind++;
	osi.set_time_mul(time_mul_des[time_mul_ind] / 300);
	const text = `${time_mul_des[time_mul_ind]}ms`;
...
let vertical_ch1_input_on = 0;
let vertical_ch2_input_on = 0;
...
function minus_trigger_level() {
	...
	const arrow_pos = osi.level * -28 + 50;
